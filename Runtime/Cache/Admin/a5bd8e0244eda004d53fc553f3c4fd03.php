											...
											<table id="sample-table-1" class="table table-striped table-bordered table-hover">
												...
												<tbody>
												 <?php if(is_array($backups)): $i = 0; $__LIST__ = $backups;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$val): $mod = ($i % 2 );++$i;?><tr class="collapsed" >
										                <td>
										                    <span style="padding-left: 20px" name="<?php echo ($val["name"]); ?>" class="expander" id="<?php echo ($val["id"]); ?>"onclick="showC(this)"></span>
										                    <?php echo ($val["name"]); ?>
										                </td>
										                <td><?php echo ($val["total_size"]); ?>kb</td>
										                <td><?php echo ($val["date_str"]); ?></td>
										                <td>
										                    <a onclick="if(confirm('确认删除?')) location.href='<?php echo U('Restore/del', array('backup'=>$val['name']));?>'" href="javascript:void(0);"  >删除</a> | 
										                    <a onclick="if(confirm('确定恢复吗?')) location.href='<?php echo U('Restore/import', array('backup'=>$val['name']));?>'" href="javascript:void(0);" >恢复</a>
										                </td>
										            </tr>
										            <?php if(is_array($val['vols'])): $i = 0; $__LIST__ = $val['vols'];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vol): $mod = ($i % 2 );++$i;?><tr id="tr<?php echo ($val["id"]); ?>" style="display:none">
										                <td><?php echo ($vol["file"]); ?></td>
										                <td><?php echo ($vol["size"]); ?>kb</td>
										                <td><?php echo ($val["date_str"]); ?></td>
										                <td>
										                    <a href="<?php echo U('Restore/download', array('backup'=>$val['name'], 'file'=>$vol['file']));?>">下载</a>
										                </td>
...
#sample-table-1 tr.collapsed td .collaps{
	background: url(/Public/admin/img/tv-collapsable.gif) center center no-repeat;
}
#sample-table-1 tr.collapsed td .expander{
	background: url(/Public/admin/img/tv-expandable.gif) center center no-repeat;
}
</style>
...
$(function(){
    $(".show_sub").click(function(){
        $(this).attr("src",function(){
            if(this.src == '_/Public/admin/img/tv-expandable.gif'){
                return '/Public/admin/img/tv-collapsable.gif';
            } else {
                return '/Public/admin/img/tv-expandable.gif';
            }
